...
namespace PALMS.Settings.ViewModel.Common
{
    public class FinsTcp
    {
        ...
        public int GetTakeoutStartdelay()
        {
            object[] result = new object[1];
            this.readWord("DM.505.1", ref result);
            return Convert.ToInt32(result[0]);
        }

        public int SetTakeoutStartdelay(int num)
        {
            int num1 = this.writeWord("DM.505.1", new object[1]
            {
        (object) num
            });
            if (num1 != 0)
        ...
        public int GetTakeoutStopdelay()
        {
            object[] result = new object[1];
            this.readWord("DM.501.1", ref result);
            return Convert.ToInt32(result[0]);
        }

        public int SetTakeoutStopdelay(int num)
        {
            int num1 = this.writeWord("DM.501.1", new object[1]
            {
        (object) num
            });
            if (num1 != 0)
        ...
        {
            object[] objArray = this.Getmemory(memory);
            return (int)this.PLC.CmdRead(this.PlcHandle, (PlcClient.PlcMemory)objArray[0], PlcClient.DataType.INT16, Convert.ToUInt16(objArray[1]), Convert.ToUInt16(objArray[2]), ref result);
        }
        ...
        {
            object[] objArray = this.Getmemory(memory);
            return (int)this.PLC.CmdWrite(this.PlcHandle, (PlcClient.PlcMemory)objArray[0], PlcClient.DataType.INT16, Convert.ToUInt16(objArray[1]), Convert.ToUInt16(objArray[2]), ref sendWord);
        }
        ...
        private object[] Getmemory(string memory)
        {
            ...
            {
                case "CIO":
                    objArray[0] = (object)PlcClient.PlcMemory.CIO;
                    break;
                case "WR":
                    objArray[0] = (object)PlcClient.PlcMemory.WR;
                    break;
                case "DM":
                    objArray[0] = (object)PlcClient.PlcMemory.DR;
                    break;
                case "HR":
                    objArray[0] = (object)PlcClient.PlcMemory.HR;
                    break;
                case "TIM":
                    objArray[0] = (object)PlcClient.PlcMemory.TIM;
                    break;
                case "CNT":
                    objArray[0] = (object)PlcClient.PlcMemory.CNT;
                    break;
                default:
                    objArray[0] = (object)PlcClient.PlcMemory.ER;
                    break;
            }
